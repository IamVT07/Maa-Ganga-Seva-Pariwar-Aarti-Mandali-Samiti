...
module.exports = async (req, res) => {
  ...
  try {
    ...
    // ── POST: Donation approve karna + Safe EmailJS Server-Side Hit ──
    if (req.method === 'POST' && action === 'approve') {
      ...
      // 2. Pure Server Side Fetch Call to EmailJS REST API (100% Keys hidden)
      if (donationData && donationData.donorEmail) {
        const emailjsResponse = await fetch('https://api.emailjs.com/api/v1.0/email/send', {
          ...
          body: JSON.stringify({
            ...
            accessToken: process.env.EMAILJS_PRIVATE_KEY, // Yeh key kisi user ko nahi dikhegi
            template_params: {
        ...
        if (!emailjsResponse.ok) {
          ...
          console.error('❌ EmailJS Server-Side Error:', errText);
          // Don't throw, return success status true since firestore updated successfully
        } else {
          console.log('✅ Thank you Email sent successfully via Backend API');
        }
      }
